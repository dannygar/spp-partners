...
namespace Spp.Presentation.User.Client.UserControls
{
    ...
    public sealed partial class CameraControl : UserControl
    {
        ...
        public CameraControl()
        {
            ...
            this.ShowDialogOnApiErrors = false;
        }
        ...
        public async Task StartStreamAsync(bool isForRealTimeProcessing = false)
        {
            try
            {
                if (captureManager == null || captureManager.CameraStreamState == CameraStreamState.Shutdown)
                {
                    ...
                    var selectedCamera = allCameras.FirstOrDefault(c => c.Name == CSSettingsHelper.Instance.CameraName);
            ...
            {
                await FaceRecognitionHelper.GenericApiCallExceptionHandler(ex, "Error starting the camera.");
            }
        }
        ...
        public async Task<ImageAnalyzer> CaptureFrameAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    await FaceRecognitionHelper.GenericApiCallExceptionHandler(ex, "Error capturing photo.");
                }
            }
            finally
            {
        ...
        {
            ImageCaptured?.Invoke(this, imageWithFace);
        }
