...
namespace Spp.Application.Services.Tests.Unit
{
    ...
    public class SessionTests : IClassFixture<DataInitFixture>
    {
        ...
        public async void GetSessionsByDateTest(string sessionDate)
        {
            try
            {
                var sessions = await _service.GetSessionsByDate(DateTime.Parse(sessionDate));
        ...
        public async void GetSessionsByDateRangeTest(string startDate, string endDate)
        {
            try
            {
                var sessions = await _service.GetSessionsRange(DateTime.Parse(startDate), DateTime.Parse(endDate));
                Assert.NotNull(sessions);
            }
            catch (Exception e)
            {
